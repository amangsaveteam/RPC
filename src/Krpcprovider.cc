...
//注册服务对象及方法，以便服务端能够处理客户端的RPC请求
void KrpcProvider::NotifyService(google::protobuf::Service *service){
    //服务端需要知道客户端想要调用的服务对象和方法，
    //这些信息会保存在一个数据结构(如ServiceInfo)中
    ServiceInfo service_info;

    //参数类型设置为google::protobuf::Service,是因为所有由protobuf生成的服务类
    //都继承自google::protobuf::Service,这样我们可以通过基类指针指向子类对象
    //实现动态多态
    ...
    //通过ServiceDescriptor,我们可以获取该服务类中定的方法列表，
    //并进行响应的注册和管理
...
void KrpcProvider::Run(){
    ...
    ZkClient zkclient;
    zkclient.Start();//连接到Zookeeper服务器
    ...
    for(auto &sp : service_map)
    {
        ...
        for(auto &mp : sp.second.method_map)
        {
            ...
            sprintf(method_path_data,"%s:%d",ip.c_str(),port);//将IP和端口信息存入节点
